...
Used for merging a candidate Wayback `wget` folder structure into the main folder structure. 
'''
...
def set_oldest(f0, f1):
    ...
    s0 = stat(f0)
    s1 = stat(f1)
    if s0.st_atime != s1.st_atime or s0.st_mtime != s1.st_mtime:
        for f in (f0, f1):
            utime(f, (min(s0.st_atime, s1.st_atime),min(s0.st_mtime, s1.st_mtime)))
...
def remove_or_set_oldest(ofname, dfname, set_oldest=False, verbose=False):
    if verbose:
...
def merge(origin, dest, remove_only=False, verbose=False, ignore_git=False, ignore_flags=False,
          continue_on_error=False, pre_hash=False, follow_links=False, set_oldest=False):
    '''
    ...
    ignore_flags :
    continue_on_error:
    follow_links: Follow links to content (destination)
    set_oldest: Set mtime & atime to oldest of origin/destination
    pre_hash: Use pre-hashing. This is often useful for large files on magnetic disks as it reads the data in a nicer
            pattern. However, it requires reading all the input files, so it may result in more IO overall.
    '''
        ...
        try:
            if not path.lexists(dfname):
                ...
                if not path.islink(dfname):
                    ...
                    yield remove_or_set_oldest(ofname, dfname, set_oldest=set_oldest, verbose=verbose)
                else:
                    print('Mismatched link: {}'.format(ofname))
            elif dir_obj.is_dir():
                if ignore_git and (dir_obj.name == '.git' or dir_obj.name.endswith(b'/.git')):
                    print('Skipping .git directory: {}'.format(ofname))
                else:
                    n = 0
                    for p_s in sorted(
                            [(path.join(basedir, dir_obj.name), subdir) for subdir in os.scandir(ofname)],
                            key=(lambda sc: sc[1].name)
                    ):
                        _,s = p_s
                        if pre_hash and s.is_file():
                            n += 1
                            lazy_hash_file(s.path)
                        filequeue.append(p_s)
            elif not dir_obj.is_file():
                print('Ignoring non-file non-directory: {}'.format(ofname))
            elif not ignore_flags and props_for(ofname) != props_for(dfname):
                print('Flags differ: {}'.format(dir_obj.name))
            elif path.isdir(dfname):
                print('File `{}` matches directory `{}`'.format(ofname, dfname))
            elif not path.isfile(dfname) and not (follow_links and path.islink(dfname)):
                print('File `{}` matches non-file `{}`'.format(ofname, dfname))
            elif not same_file_content(ofname, dfname, pre_hash):
...
def parse_options(argv):
    ...
    parser.add_option(
        '--use-pre-hash', action='store_true', dest='pre_hash',
        help='Use pre-hashing. This is often useful for large files on magnetic disks as it reads the data '
             'in a nicer pattern. However, it requires reading all the input files, so it may result in more'
             ' IO overall.'
    )
...
def main_merge(src, dest, *args, **kwargs):
    for op in merge(src.encode('utf-8'), dest.encode('utf-8'), *args, **kwargs):
        try:
            op.run()
